    ...
    for device in PhoneDevice.objects.all():
        ...
        try:
            ...
            if not phonenumbers.is_valid_number(number):
                logger.info("User id='%s' has an invalid phone number '%s'." % (user_id, device.number))
            ...
            # so do log a warning.
            logger.warning("User id='%s' has an invalid phone number '%s': %s. Please resolve this issue, "
                           "as it might result in errors." % (user_id, device.number, e))
